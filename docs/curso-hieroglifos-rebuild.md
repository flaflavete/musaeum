...
**Lição 4 — Escrevendo em egípcio antigo (feito e verificado):** bloco novo `builder` no motor, uma versão **guiada** do construtor (não o livre do `gardiner.html`): paleta de sinais do `GARDINER_DATA`, cada desafio dá significado + transliteração, o aluno clica os sinais na ordem e recebe feedback na hora (acerto verde com nota do complemento; erro pede conferir a ordem). 4 desafios (pr, mn, nfr, ꜥnḫ) que revisitam sinais das lições 1 a 3. Sem quiz: a lição conclui ao montar as 4 palavras. `wireBuilder` guarda o estado; ⌫ Apagar / Limpar.
...
**AS 6 LIÇÕES ESTÃO PRONTAS.** O curso novo em `curso/` está completo e verificado ponta a ponta (1 O sistema, 2 Os 24 unilíteros, 3 Bi/trilíteros, 4 Escrevendo em egípcio, 5 Cartuchos, 6 Ler um texto mágico (fórmula de oferenda)).
